...
<div class="mx-auto max-w-2xl">
    <div class="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">
                Perbarui identitas dan peran akses untuk {{ $user->name }}.
            </p>
        </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <form method="POST" action="/users/{{ $user->id }}" class="p-5 sm:p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-5">
                <div>
                    ...
                    <input id="name" name="name" type="text" value="{{ old('name', $user->name) }}"
                           ...
                           class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-3 text-sm text-slate-800 shadow-sm outline-none transition focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100 @error('name') border-rose-400 focus:border-rose-500 focus:ring-rose-100 @enderror dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-cyan-400 dark:focus:ring-cyan-400/15">
                    @error('name')
                ...
                <div>
                    ...
                    <input id="username" name="username" type="text" value="{{ old('username', $user->username) }}"
                           ...
                           class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-3 text-sm text-slate-800 shadow-sm outline-none transition focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100 @error('username') border-rose-400 focus:border-rose-500 focus:ring-rose-100 @enderror dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-cyan-400 dark:focus:ring-cyan-400/15">
                    @error('username')
                        <p class="mt-2 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input id="email" name="email" type="email" value="{{ old('email', $user->email) }}"
                           ...
                           class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-3 text-sm text-slate-800 shadow-sm outline-none transition focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100 @error('email') border-rose-400 focus:border-rose-500 focus:ring-rose-100 @enderror dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-cyan-400 dark:focus:ring-cyan-400/15">
                    @error('email')
                        <p class="mt-2 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <p class="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-400">
                        Peran Super Admin tidak dapat diubah dari halaman ini.
                    </p>
                    @error('role')
                        <p class="mt-2 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>
                    @enderror
